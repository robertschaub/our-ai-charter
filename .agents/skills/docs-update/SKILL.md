...
1. **Status banner.** A top-of-file `> **Status: …**` banner belongs on the **normative** docs (Founding Accord, evaluation protocol, certification model, one-pager) and the **outreach/parliamentary** drafts (Non-Paper, Aktionsplan, Postulat), labelled `DRAFT`; on `PUBLISHED <date>` pages; and on the `docs/wip/` notes, labelled `WORKING NOTES`. Briefings, evidence, strategy, and background carry none. Confirm the banner is present and accurate where it belongs; if maturity changed, update the label.
2. **README index.** Open the [README](../../../README.md). It presents **Our AI Charter / the Public AI Network** (this whole effort) — its overview, compute pillar, published articles, and the strategy/outreach/evidence — with **AI Assurance & Certification** as the nested trust sub-stream, each document's status labelled inline. Move or add the document to the right section with a one-line description that matches its current framing. Remove stale entries.
