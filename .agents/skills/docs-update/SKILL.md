...
1. **Status banner.** A top-of-file status banner — a plain `> **Status: …**` blockquote (not a typed callout) — belongs on **normative** docs and **outreach/parliamentary** drafts, labelled `DRAFT`, and on `PUBLISHED <date>` pages — see [AGENTS.md](../../../AGENTS.md). Evidence, background, navigation, and other non-normative pages carry no banner. This public repository has no WIP or archive section; every current public document needs a durable subject home. The README index notes each document's status either way.
