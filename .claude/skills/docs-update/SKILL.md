...
After you add, rename, substantially edit, **promote** (e.g. `WORKING DRAFT` → `PUBLISHED`), or retire any `.md` document under `manifesto/`, `charter/`, `protocol/`, or `background/`.
...
2. **README index.** Open the [README](../../../README.md). Its index is grouped by status (Published / Working drafts / Discussion & background). Move or add the document to the correct group with a one-line description that matches its current framing. Remove stale entries.
