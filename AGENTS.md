...
- **Status banners.** Every content page carries a status banner at the top — a plain `> **Status: …**` blockquote, **not** a typed callout (the `[!NOTE]` / `[!TIP]` / `[!WARNING]` headers read as advice or hazards, which is wrong for a status label). There are **three labels**: `PUBLISHED <date>` (ISO `YYYY-MM-DD`) for published pages; `DRAFT` for anchored, curated work not yet published (the Founding Accord and the other normative drafts, the outreach and parliamentary materials, briefings, evidence, strategy, background); and `WORKING NOTES` for raw material not yet anchored into the curated structure — discussion dumps and scratch (the `docs/wip/` notes). Only the **navigational** pages (the home page, Start here, the program map) carry **no** banner. Keep labels accurate; the README index also notes each document's status. Reserve `> [!WARNING]` / `> [!CAUTION]` for genuine in-body hazards, never for status.
...
- Public issues and pull requests are enabled for feedback. The website is built with **MkDocs Material** from the Markdown under `docs/` and published to GitHub Pages by [`.github/workflows/deploy-docs.yml`](.github/workflows/deploy-docs.yml) on every push to `main` (Pages source = "GitHub Actions"). The Markdown documents stay authoritative — the site is a presentation layer over them. Every content page shows its status as a top-of-page `> **Status: …**` banner (`PUBLISHED`, `DRAFT`, or `WORKING NOTES`); navigational pages carry none. The notes under `docs/wip/` are part of the built site, grouped under a *Working notes* nav section and flagged `WORKING NOTES`. Wiki and Projects should stay disabled. GitHub Actions must stay enabled for the Pages build (official `actions/*` plus `mkdocs build`); editing that build pipeline is fine, but do not add unrelated workflows unless the maintainer requests them.
