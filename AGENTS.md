...
- **Use Git for versioning.** Keep published article filenames stable, and keep each article's accompanying feed post, picture, article, and comments **in that same file** — not as separate `*-post.md` / `*-comment.md` mirrors. The opening order is publication line → post → picture → article, with exactly three `***` thematic-break lines between those four blocks and no redundant section labels or additional thematic rules; comments may follow the article under concise labelled sections. Render hashtag rows as ordinary text by escaping the first `#`, never as headings or inline code. Keep the picture directly between its two `***` lines without blank source lines. Do not use adjacent HTML `<hr>` tags: GitHub-flavored Markdown can treat the following image and heading as raw block content. Do not create dated or versioned copies such as `*-v2.md` or `*-2026-06-16.md`. Use Git history, the compact publication line, changelog entries, and necessary document notes to record versions and changes.
