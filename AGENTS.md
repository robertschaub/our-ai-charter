...
- **Status banners** sit at the top of three kinds of page only: pages that state **rules, obligations, or guidelines** (the Charter's normative docs — Founding Accord, the evaluation protocol, the certification model, the one-pager); **`PUBLISHED`** pages; and the **outreach and parliamentary materials** kept as working drafts (the Non-Paper, the Aktionsplan, and the Postulat). The label — `WORKING DRAFT`, `WIP / DISCUSSION`, or `PUBLISHED <date>` (ISO `YYYY-MM-DD`) — is a plain `> **Status: …**` blockquote, **not** a typed callout (the `[!NOTE]` / `[!TIP]` / `[!WARNING]` headers read as advice or hazards, which is wrong for a status label). All other pages (briefings, evidence, strategy, background, the program map) carry **no** banner. Keep labels accurate where they exist; the README index still notes each document's status. Reserve `> [!WARNING]` / `> [!CAUTION]` for genuine in-body hazards, never for status.
...
- Public issues and pull requests are enabled for feedback. The website is built with **MkDocs Material** from the Markdown under `docs/` and published to GitHub Pages by [`.github/workflows/deploy-docs.yml`](.github/workflows/deploy-docs.yml) on every push to `main` (Pages source = "GitHub Actions"). The Markdown documents stay authoritative — the site is a presentation layer over them. Provisional pages are flagged on the site with Material's page-status feature: set `status: wip` (WIP / discussion) or `status: draft` (working draft) in a page's YAML front matter and the theme renders a nav-sidebar icon (`theme.icon.status` / `extra.status` in [mkdocs.yml](mkdocs.yml)). This is **metadata only** — it adds no in-body banner, so it is compatible with the banner rule above and leaves the GitHub view unchanged; published pages carry no status icon. Working scratch under `docs/wip/` is kept out of the built site with `exclude_docs: wip/` (it stays in the repo and Git history). Wiki and Projects should stay disabled. GitHub Actions must stay enabled for the Pages build (official `actions/*` plus `mkdocs build`); editing that build pipeline is fine, but do not add unrelated workflows unless the maintainer requests them.
