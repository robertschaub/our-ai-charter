...
- Keep status banners, the README index, and CHANGELOG in sync — see the `/docs-update` skill. Every content page carries a top-of-page `> **Status: …**` banner with one of three labels — `PUBLISHED <date>` (ISO `YYYY-MM-DD`), `DRAFT` (anchored, curated, not yet published), or `WORKING NOTES` (raw/unanchored — discussion dumps, scratch); only navigational pages (home, Start here, program map) carry none (see [AGENTS.md](AGENTS.md)). When the maintainer says a doc is "Published", set its banner to `PUBLISHED <date>` (today's date if publishing now).
