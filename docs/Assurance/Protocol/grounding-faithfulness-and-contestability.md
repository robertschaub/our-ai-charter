...
| A **source‑validity floor** (a cited source must exist and not be fiction/satire — §4.1), plus a **source‑standing marker** for claim classes the use-case profile enumerates in advance (§4.1). | Full **source‑reliability grading** — *credibility*, as distinct from standing: a known gap, held as a *separate future module*, not silently smuggled in (§9). Standing for **un-enumerated** claim classes, which is reported `not assessed`. |
...
**Source standing — enumerated per use-case, then recorded.** Standing asks whether a source was *entitled* to establish a claim: a court for its own judgment, a register for what it registers, a statistical office for its own release. Deciding that per claim would be a domain judgment, so the protocol does not ask raters to make one. Instead, **the use-case profile enumerates, in advance, which claim classes have an entitled source and what it is.** Only enumerated classes are assessed; everything else is `not assessed` for standing, and the report says how much of the sample that covers.

For an enumerated class, each citation records one of five states — the **entitled source**; a **report of** it; **not entitled** (an entitled source exists and this is not it); **no entitled source** (the class has none, so the field carries no adverse signal); or **unattributed** (standing could not be determined). It never changes the support score: a claim resting only on `not entitled` or `unattributed` is **reported separately**, not scored unsupported, because standing is not a truth call.

**The honest cost.** Building an enumeration is real per-use-case domain work, done once, and it is where the judgment lives — moved to a reviewable artefact and out of the per-claim rating loop. Applying it afterwards is close to mechanical. What this does *not* do is remove the judgment; a wrong or contested enumeration produces wrong standing markers across the whole sample, which is why the enumeration is published with the report and can be argued with.
...
6. **Re‑evaluation** at least annually, **more often for frequently‑updated systems**, after assessor-reviewable material updates or material incidents, plus complaint‑triggered spot checks.
7. **Status withdrawn** (with reason, kept visible) if: re‑evaluation drops below bar · the correction process is found curated · the objection channel is shown to be theatre · a material restriction/shutdown power was hidden, misrepresented, or cannot be independently reviewed · the evaluated version is silently swapped · the release risk assessment is materially false, incomplete, or over-redacted · the report is cited beyond its stated scope.
...
- **(b) Governance transparency — all systems.** A public statement of *who* can restrict, manipulate, recall, or shut the system down, on *what lawful basis*; material restrictions, recalls, shutdowns, or external orders recorded with date, scope, duration, legal or contractual basis, issuer/category of issuer, decision-maker, and review route where disclosure is lawful. If disclosure is lawfully restricted, the evidence is retained for confidential assessor or oversight review where legally possible. If neither public disclosure nor confidential independent review is possible, the affected scope cannot keep the pilot listing or future claim.
...
- **Standing is newly separated from credibility, and its coverage is narrow.** Whether a source was *entitled* to establish a fact is a different question from whether it is *credible*. v0.2 did not conflate them — it was **silent** on entitlement and deferred reliability only; the omission was that standing was never named as a distinct question at all. v0.3 names it and assesses it, but only for claim classes an enumeration covers (§4.1), so a report can legitimately show standing as `not assessed` for most of its sample. Three limits stand: standing shows that the right party spoke, never that what they said is true; many claim classes have no entitled source, where the field is deliberately signal-free; and the enumeration itself is a judgment that can be wrong at scale. Credibility grading remains deferred to a separate module.
...
- **A voluntary report is not law.** States set enforceable rights and sanctions, unevenly across jurisdictions. This protocol does not certify legal compliance; it contributes one public-interest module to the assurance stack and — once evaluations run — would turn part of the operational duties into usable evidence for buyers and the public ahead of binding enforcement, complementing the EU AI Act and the Council of Europe AI Convention.
