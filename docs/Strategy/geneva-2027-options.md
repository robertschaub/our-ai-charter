...
2. **Produce the blueprint — Route C.** A neutral pre-sprint turns the idea into a memo others can carry. The **domestic cross-party sprint** now leads with the goal itself — an **international network of open, charter-bound AI models** that Switzerland can catalyse with a national impulse (the vehicle — a policy sprint, an impulse programme in the sense of motion 26.3221, or another — is left open, not pre-set as the goal), Switzerland as hub and coordinator; the coordinating role is aspired/earned, not a mandate claimed today (motion-anchored — see [action plan](../Outreach/action-plan.en.md)); positive framing (host, resilience, self-determination) carries the headline. Keep the harder **capture-resistance / co-stewardship** wording as separate, Geneva-framed detail.
