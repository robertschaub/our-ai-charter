...
**Current focus — Evidence-Gated Agents.** Robert Schaub is defining and preparing the first EGA prototype. The [requirements](wip/evidence-gated-agents-requirements.md) and [specification](wip/evidence-gated-agents-spec.md) are public working drafts; the implementation baseline remains to be agreed.
...
**Proposed next step.** Subject to funding and an agreed scope, a bounded German/English prototype would connect FactHarbor and the runtime to check answers against authority and prepared public evidence before release, record decisions and delivery outcomes, and support bounded challenges and correction. Answer delivery would be its only real action type. See the [proposed prototype scope](Assurance/Concepts/evidence-gated-agents.md#proposed-prototype-the-answer-delivery-path).
