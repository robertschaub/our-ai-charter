...
Those gates are also moments to [test AI-supported power](Published/a-practical-test-for-power.md#test-every-use-of-power) and reopen dialogue. Ask who has authority, who benefits and bears the cost, who was heard, whether expected harm is necessary and proportionate and a less harmful path exists, whether affected people can safely disagree, refuse, or challenge and obtain independent review and remedy, and which limits must hold even when they frustrate the goal. Check both specific people and population patterns: individually admissible actions can accumulate into a discriminatory or otherwise unauthorized effect.
...
No body should control the accountability chain. Service roles — model or data provider, deployer, node operator — would remain distinct from the rulemaker, record keeper, independent reviewer, and remedy decider ([why the roles must stay separate](Published/when-vs-who-ai-governance.md)). If the scheme matured, accreditation and peer review would check the reviewers.
...
The nearest live starting pattern is the Public AI Inference Utility's Apertus-backed access layer. The proposed pilot would add an evaluation service and broker-and-evidence controls, then federate a second node or provider and ideally a second model family. These are reference components, not an affiliation or active-pilot claim. Node control would stay local while identity, policy, quota, accounting, evidence, and accountability are shared.
...
Commitment would also need an enforceable consequence. The proposed mechanism is access rather than general policing: current assurance status would gate shared compute or corpus access and any procurement preference; failure, expiry, or a material false claim would suspend the relevant status or benefit, subject to notice and appeal. No participant would control its own audit. Without a consequence, a commitment is only a declaration.
