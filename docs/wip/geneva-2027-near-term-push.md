> **Status: WORKING NOTES** — near-term action plan from a July-2026 prioritisation session. Supersede or delete after mid-July (the **12 Jul** Swissnex window and the first Public AI contact). Canonical detail lives in the linked docs; this note is the index + live checklist.

# Geneva 2027 — near-term push (July 2026)

What's most urgent and most important across the [Geneva 2027 routes](../Strategy/geneva-2027-options.md) right now, and the concrete July actions. Detail lives in the linked docs — this note ties them together with the live decisions and a checklist.

## Priorities

- **Most urgent:** the [Swissnex "Geneva Loading…" fellowship](../Strategy/geneva-2027-swissnex-fellowship.md) channel — **window closes 12 Jul 2026**.
- **Most important:** the **Public AI / Metagov** relationship ([Route B](../Strategy/geneva-2027-options.md)) — the priority public-AI relationship thread — and the **official summit process** ([Route A](../Strategy/geneva-2027-options.md)), which the fellowship side-event ultimately plugs into.

## Confirmed dates

| Date | What |
|---|---|
| **7–10 Jul 2026** | AI for Good Global Summit, Palexpo Geneva — context only; **no attendance planned and no outreach depends on it** |
| **12 Jul 2026** | Swissnex fellowship window closes |
| ~mid-Jul 2026 | Expedition Zukunft scoping follow-up ([Route C](../Strategy/geneva-2027-options.md)) |
| early Aug 2026 | Swissnex notification |
| Sept 2026 | Swissnex prep day, CH (date TBD) |
| **16–20 Nov 2026** | Swissnex SF immersion week — **feasible; gate cleared** |
| first half 2027 (TBA) | Geneva AI Summit + fellowship side-event |

## The July work

**Swissnex — window closes 12 Jul.** The programme's entry format (four short free-text answers, ~1,050 words, plus commitment questions) is in the [fellowship note](../Strategy/geneva-2027-swissnex-fellowship.md); any output must be reachable from a public link. The "specific output" would draw on the [keystone memo](geneva-2027-keystone-memo.md).

**Public AI / Metagov — direct, asynchronous contact.** Joshua Tan is the current bridge across Public AI, its inference utility and Current AI; his [recent update](https://www.linkedin.com/posts/joshuaztan_a-year-ago-public-ai-was-just-a-white-paper-activity-7458334500197048320-gZZm) and the open [Public AI Fellow, Switzerland](https://publicai.network/jobs/fellow-switzerland/) role make the Switzerland track an active, evidenced opening. Contact him directly with one bounded question: would a governance-and-evidence contribution complement the track or duplicate work underway? A publicai.ch introduction remains useful but is not a prerequisite. Do not imply an AI for Good meeting: no attendance is planned and there is no evidence Tan will be there.

## Parallel routes — status

- **A — official process:** monitor dig.watch / ICT4Peace for the next input window (undated); keep the keystone memo submission-ready.
- **B — Public AI / Metagov:** open the conversation asynchronously now — it doubles as a substance conversation for the Geneva channels and resolves the naming question ([Apertus plan §4](apertus-fit-and-engagement-plan.md)).
- **C — Expedition Zukunft:** follow up on the neutral-convenor scoping (~mid-July).
- **E — political flank:** watch National Council handling of motions [24.3209 / 26.3221](../Strategy/digital-sovereignty-motions.md).

## Next actions

- [ ] Keep the keystone memo / package submission-ready for the next open Geneva channel.
- [ ] Send the grounded direct note to Joshua Tan; optionally seek a parallel introduction via publicai.ch.
- [ ] Expedition Zukunft scoping follow-up (~mid-July).

## Open decisions

- Embrace / subordinate / rename the "Public AI Network" term — resolve once the Public AI/Metagov relationship is tested ([Apertus plan §4](apertus-fit-and-engagement-plan.md)).
