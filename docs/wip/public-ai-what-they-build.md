...
- **Three operating layers, one thin legal core.** The *network* (publicai.network) is a coalition with no legal entity; the *Inference Utility* (publicai.co) is a nonprofit open-source project fiscally sponsored by [Metagov](https://projects.propublica.org/nonprofits/organizations/853442527) (US 501(c)(3); latest filed 990: FY2023 revenue <$400k); the *chapters* (CH/US/UK) are three unlike experiments, and only the Swiss one has a published legal vehicle ([§1](#1-the-shape-of-the-thing-one-movement-three-operating-layers)).
- **The utility is real and running** — chat + OpenAI-compatible API serving open models from five national or regional origins (CH · SG · US · PL · EU), official [Hugging Face Inference Provider](https://huggingface.co/blog/inference-providers-publicai), serving stack open source under [`forpublicai`](https://github.com/forpublicai) ([§3](#3-what-actually-runs-the-product-surface)–[§6](#6-the-model-catalog-as-of-2026-07-27)).
- **But it is smaller than it reads:** disclosed lifetime cash ≈ **$126k** on the [Open Collective ledger](https://opencollective.com/publicai); compute is donated or credit-funded; the public contributor graphs show a single engineer carrying most commits; usage counters are self-reported ([§4](#4-under-the-hood-the-verified-serving-architecture), [§5](#5-compute-who-serves-vs-who-donated), [§9](#9-the-disclosed-money), [§11](#11-claims-audit)).
- **Public AI Switzerland is legally a pre-cooperative:** published statutes name a founding date of **11 Aug 2026** (still ahead); no commercial-register entry was findable as of 2026-07-27; dues sit in trust with Datalets.ch; the site's "52,293 registered users" most plausibly — my read, [§7](#7-public-ai-switzerland-the-cooperative-precisely) — counts utility-wide chat users, not Swiss members.
...
| **Funding models** — state capital, philanthropy with anti-capture terms, at-cost usage fees, cooperative structures | Sustainable public financing | Experiments only: donated compute + ~$126k disclosed cash + wallet-credit billing + CHF-100 cooperative shares + $1,000/yr kiosk fees ([§9](#9-the-disclosed-money)) |
...
- **API** — `api.publicai.co`: OpenAI-compatible (`/v1/chat/completions`, `/v1/models`), self-service keys via the [developer portal](https://platform.publicai.co/) (Auth0). Four rate tiers: Free 100 req/min · Plus 200 (via Open Collective contribution) · Pro 300 · Enterprise 10,000 (by email). Token usage draws down **wallet credits** (Lago billing + top-up); new accounts get starter credits (docs in the [platform repo](https://github.com/forpublicai/platform.publicai.co)). Metering and monetisation are later additions — the Sep 2025 [HF launch post](https://huggingface.co/blog/inference-providers-publicai) still described free access at 20 req/min (for the route it announced).
...
- **Routing is federated and weighted.** The same model name is declared across several providers with weights (CSCS preferred for Apertus); failures cool a lane down for 60s and retry elsewhere; **fallbacks cross borders and models** (e.g. Apertus-70B → Qwen-SEA-LION-32B) — a resilience feature that is both a border crossing and a silent model substitution, in tension with jurisdiction and which-model promises alike ([model YAMLs in the chat repo](https://github.com/forpublicai/chat.publicai.co)).
...
- An **inference-partner program** now formalises donations: partners contribute compute, receive attribution, onboard via a runbook (FAQ in the [platform repo](https://github.com/forpublicai/platform.publicai.co)).
- **My read:** "has donated" ≠ "is serving." The live federation today is CH-heavy (three of six lanes) plus Singapore and Poland — the sixth lane, Bedrock (Frankfurt), carries embeddings only; the launch's five-country GPU pool was a moment, not a steady state — and the pivot to CPU inference is the honest economic signature of donation-shaped compute.
...
- **Catalog drift is visible:** Spain's ALIA/Salamandra were served earlier on the Intel CPU layer and still appear in front-page copy, but are absent from the current router config — parked, dropped, or served outside the public config; the sources don't say. Historical lanes (Llama-3.2 via Bedrock, Mistral) have been removed. [Self-reported copy vs config; discrepancy noted.]
...
- **Legal status.** The site self-describes as "the world's first cooperative for AI", to be a Genossenschaft under Art. 828 ff. OR; the published [statutes](https://publicai.ch/statuten.html) name **"Genossenschaft Public AI", seat Liebefeld** (Köniz BE), and state the statutes were fixed at the founding on **11 Aug 2026 — a date still ahead** at verification time. (The informal label **"SPIU"** — *Swiss Public Inference Utility* — predates the statutory name and survives in third-party text and as a Hugging Face handle.) The [impressum](https://publicai.ch/impressum.html) carries no UID and says formal establishment is under way; terms and privacy pages are marked drafts pending registration. A name-substring query against the federal LINDAS mirror of the Zefix commercial register on 2026-07-27 returned no entity containing "public ai" — a check that does not exclude one-word spellings or mirror lag, but is consistent with the site's own "still being set up"; the authoritative test is a fresh Zefix/SHAB lookup after 11 Aug. Founding dues are held in trust by [Datalets.ch](https://datalets.ch/) — per the Swiss commercial register a sole proprietorship of Oleg Lavrovsky, not a nonprofit (see the [actor directory](../Evidence/actors-and-landscape.md)); publicai.ch nonetheless describes it as a Swiss nonprofit association.
...
- **What it operationally is (my read):** a Swiss front for the utility, today. The service (chat + API) is operated by the Metagov-hosted utility; the cooperative adds Swiss framing, membership, and a governance promise. ETH's Apertus release positioned the utility as access **for people outside Switzerland** (Swisscom being the Swiss business channel) ([ETH press release](https://ethz.ch/en/news-and-events/eth-news/news/2025/09/press-release-apertus-a-fully-open-transparent-multilingual-language-model.html)); publicai.ch markets the same utility **inside** Switzerland to consumers and SMEs — a niche not explicitly endorsed on the partner-side pages checked (ETH's release, ai-weeks.ch; 2026-07-27).
- **Activity record since the community founding event** (26–27 Feb 2026, Zürich — distinct from the *statutory* founding assembly set for 11 Aug 2026; the pre-published [meeting agenda](https://publicai.ch/meeting/27.2.2026/) included a board vote, but no minutes or board names are published on the site as of 2026-07-27; the public [Luma page](https://luma.com/3fwdxk84) shows 10 RSVPs for day two): founding-membership reservations opened 20 May 2026; **venture.ch Spotlight Award finalist** (theme: Apertus applications; [venture.ch/spotlight](https://www.venture.ch/spotlight)) in June 2026; notary engagement announced 17 Jul 2026; **Swiss National AI Dialogue** launched 21 Jul 2026 ([dialogue.publicai.ch](https://dialogue.publicai.ch/) — anonymous public input on Swiss AI, partners incl. Change.org, Collective Intelligence Project, Metagov; explicitly not a government project; 788 participants self-reported); Mesh Festival Basel appearance announced for 16 Oct 2026 ([news](https://publicai.ch/news.html)).
- **Claims vs verifiable.** The homepage counters — **52,293 registered users, 2,166 API developers, 9.58B tokens** — are self-reported; *my read:* most plausibly utility-wide rather than Swiss-member counts (the founding event's public RSVP list held 10 names and no membership number has ever been published; the alternative — a publicai.ch-specific registration counter — cannot be excluded from public sources). Older copy ("#1 global deployer of Apertus", "35,000+ users", "trillions of tokens") survives in the site repo's README ([forpublicai/publicai.ch](https://github.com/forpublicai/publicai.ch)) and search snippets, internally inconsistent with the live 9.58B counter (different scopes or stale copy — unresolved). The claimed Swiss {ai} Weeks partnership is not mirrored on [ai-weeks.ch](https://ai-weeks.ch/) (checked 2026-07-27), though the utility did supply free Apertus API keys to 2025 hackathon participants. **No dedicated Swiss press coverage of the cooperative was found** (searched swissinfo, NZZ, Netzwoche, Inside IT, heise in DE/EN, 2026-07-27) — the closest is Sanders & Schneier's newsletter piece calling SPIU founded ([democracyrenovator.com, 21 Feb 2026](https://www.democracyrenovator.com/p/rewiring-democracy-now-switzerland)), while incorporation was, and remains, pending.
- **Team (as listed 2026-07-27):** Anna Sotnikova, Cornelia Wolfenstädter, Daria Höhener, Joshua Tan, Oleg Lavrovsky, Prashanth Kanduri ([about](https://publicai.ch/about.html)); the earlier co-founder listing of Sabine Wildemann no longer appears there (no reason is stated publicly; she hosted the founding event and leads aiLights / co-initiated Swiss {ai} Weeks); a Swiss Hugging Face org under the SPIU handle ([huggingface.co/spiu](https://huggingface.co/spiu)) exists alongside the utility's, its operator not stated on the page.
...
- **publicai.us — Maine.** Now presents as a regional deployment: free/subsidised chat, a small-business permitting assistant, library kiosks, a planned Maine AI Dialogue, beta "community agents" that watch town halls and grants — on **MOCSI** (Maine Open Compute Services Initiative), community-governed compute seeking co-investors ([publicai.us](https://publicai.us/)).
...
Reconciliation notes: the itemised organisational contributions sum to $135k, above the ledger's displayed $126,300 lifetime total — the ledger does not break out pledged-vs-received or fees, so treat its displayed totals as authoritative and the per-contributor figures as posted amounts (all read 2026-07-27). Programme money also flows outside this ledger: the Siegel Family Endowment funds the benchmarks work via Aspen Digital, and the Creative Fellowship is CCI-funded ([§8](#8-the-rest-of-the-build-chapters-and-programs)) — hence "disclosed", not "complete".

**My read:** the disclosed cash is strikingly small against the infrastructure ambition — roughly one mid-level engineer-year of money spent over the project's life, with sustainability resting on in-kind compute, credits, a young metering system, and partner institutions' goodwill. The most consequential financial fact is the Current AI relationship: Tan's dual role (disclosed on his own page) plus role funding routed via Metagov, with no org-to-org agreement published anywhere — whether one exists, and how the dual role is governed, is a question to ask, not a finding; either way it is a key-person dependency. The utility's own about page jokes it is "three toddlers in a trench coat" — the public ledger largely corroborates the self-deprecation.
...
| Serving stack open source | **Verified for every component identified in §3–§4** (23 public repos); absence of non-public components is not externally checkable | [forpublicai org](https://github.com/forpublicai) |
...
| ETH AI Center / EPFL AI Center / Swiss {ai} Weeks partnerships | **Asymmetric** — claimed on publicai.ch; not mirrored on [ai-weeks.ch](https://ai-weeks.ch/) (checked 2026-07-27), and no partner-side confirmation was found on the other pages checked | partner sites |
...
1. **A movement with one working proof-point.** The durable asset is not the coalition (which is deliberately structureless) but the utility: a genuinely running, genuinely open, multi-country inference service that gives national open models a day-one public front door. That is a real institution-in-miniature — and by far the most complete part of the whitepaper's taxonomy the coalition itself has built (the library kiosks and benchmarks are early seedlings by comparison).
2. **An operating model of borrowed everything.** Borrowed legal skin (Metagov), borrowed compute (donations, credits, national HPC), borrowed models (Apertus, SEA-LION, OLMo…), tiny cash. Each arrangement is normal and deliberate on its own — fiscal sponsorship is a standard nonprofit form, and serving Apache-2.0 models is exactly their intended use; it is the *stack* of dependencies that makes the achievement more impressive and the permanence claim weaker. The whitepaper's third feature — *permanent* public goods — is the one the current structure least supports.
3. **Switzerland is the institutionalisation test.** The cooperative is the first attempt to give the movement a self-sustaining legal and revenue form. Until the 11 Aug 2026 founding and a register entry, it remains a membership campaign wrapped around the utility — which is also what ordinary Swiss pre-incorporation looks like (statutes published, notary engaged, dues in trust), so the founding assembly is the test, not the verdict. Worth re-checking within weeks: a registered Genossenschaft with real members would change its weight in the Swiss landscape.
4. **Accountability is the declared-but-unbuilt layer — by their own evidence.** The unanswered jurisdiction-proof issue, the self-declared serving attribution, the absent status page, the self-reported counters, the asymmetric partner claims: none of this is scandalous for a project this size, but it means that **for the utility's own operational claims** the "publicly accountable" pillar currently rests on openness plus self-reporting. The coalition's accountability-flavoured builds — the National AI Dialogue, the community benchmarks, the evaluator forum, the cooperative's one-member-one-vote design ([§7](#7-public-ai-switzerland-the-cooperative-precisely)–[§8](#8-the-rest-of-the-build-chapters-and-programs)) — sit *beside* the service rather than *over* it: none independently attests what the utility actually runs. For this repo's purposes that is the documented, primary-source version of the gap the Charter's [assurance & certification building block](../About.md) addresses — an ally's gap, not a competitor's ([positioning](metagov-public-ai-landscape.md) §4, [engagement plan](apertus-fit-and-engagement-plan.md) §5).
...
- The Apertus project domain now resolves to **apertus-ai.org** — apertvs.ai returns a 308 permanent redirect (verified directly 2026-07-27). Files still linking apertvs.ai (they resolve via the redirect): [linked-sources.md](../Evidence/linked-sources.md), [architecture.md](../Infrastructure/architecture.md), [geneva-2027-channel-demand.md](../Evidence/geneva-2027-channel-demand.md), [copyright-clean note](copyright-clean-open-llm-strategy.md). Do **not** change anything to `apertus.ai` — that remains an unaffiliated reseller.
- Catalog grew from 2 to 5 national/regional model families (+OLMo, Bielik, EuroLLM; ALIA earlier); Intel-CPU-on-AWS became the main non-CSCS lane; monetisation (wallet credits, tiers) replaced the flat free posture.
...
- Further propagation targets found by cross-checking companions: [actors-and-landscape.md](../Evidence/actors-and-landscape.md) (Wildemann entry; "no formal board published" can gain the 11 Aug 2026 assembly date), [public-ai-people-and-pathways.md](public-ai-people-and-pathways.md) (Wildemann co-founder box in the §1 diagram), [network-overview.md](../network-overview.md) (its "PublicAI / SPIU" description — §7 here: the Swiss entity operates nothing itself; the Metagov-hosted utility does), and the companions' unqualified "open Fellow (Switzerland) role" phrasing (the posting was still listed 2026-07-27 with no public appointee announcement — date the claim rather than assert openness).
