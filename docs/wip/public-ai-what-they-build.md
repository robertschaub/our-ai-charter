...
Notable in the configs: the same model is declared across providers with weights, and **fallbacks cross both borders and models** (e.g. Apertus-70B → Qwen-SEA-LION-v4-32B per [apertus-70b-instruct.yaml](https://github.com/forpublicai/chat.publicai.co/blob/main/charts/web_services/charts/litellm/models/swiss-ai/apertus-70b-instruct.yaml)) — resilience by design, in tension with jurisdiction and which-model expectations **[configured]**; whether a response discloses the substitute (stock LiteLLM behaviour suggests its `model` field may) is untested against the live API **[unverified]**. The GPU story became a CPU story: the Sep–Nov 2025 launch ran on ~$124k of AWS credits (3× 8-GPU p4d pods); when credits ended, serving shifted to Intel Xeon CPU inference in the same region ([amazon-intel story](https://publicai.co/stories/amazon-intel)) **[self-reported/partner-stated]**. Whether any self-operated GPU pods still run is **[unverified]**.
...
- **Legal status.** publicai.ch positions itself as "the world's first cooperative for AI" and as customer-owned **[self-reported]**; the published draft [statutes](https://publicai.ch/statuten.html) name **"Genossenschaft Public AI", seat Liebefeld** (Köniz BE), under Art. 828 ff. OR, with a founding date of **11 Aug 2026 — still ahead** at verification. (The informal label **"SPIU"** — *Swiss Public Inference Utility* — predates the statutory name and survives in third-party text and a Hugging Face handle.) The [impressum](https://publicai.ch/impressum.html) carries no UID; terms and privacy are drafts pending registration. A name-substring query against the federal LINDAS mirror of Zefix (2026-07-27) returned no entity containing "public ai" — a check that does not exclude one-word spellings or mirror lag; the authoritative test is a fresh Zefix/SHAB lookup after 11 Aug. Under Swiss cooperative law, formation requires at least seven founders (Art. 831 OR), a **publicly notarised founding** — required for cooperative foundings since 1 Jan 2023, per the Bern registry: «seit dem 01.01.2023 Gründungen … einer öffentlichen Beurkundung bedürfen» ([registry overview](https://www.hra.dij.be.ch/de/start/rechtsformen/genossenschaft.html); [Merkblatt](https://www.hra.dij.be.ch/content/dam/hra_dij/dokumente/de/merkbl%C3%A4tter/HRA_Merkblatt_Neueintragung_Genossenschaft_2023_de.pdf)) — and the commercial-register entry that brings the cooperative into existence. publicai.ch's [news page](https://publicai.ch/news.html) says the incorporation date is being coordinated with the notary, consistent with that requirement. Read the published statutes as **pre-adoption drafts dated 11 Aug 2026** — the statutes page itself carries no draft label; that reading rests on the future-dated closing clause and the join page's funds-held-in-trust-until-registration language. No notice of a *notarial* constitutive assembly, founder list, or membership number was identified (checked 2026-07-28; the public record of the Feb 2026 Zürich founding event is a community membership meeting, not the notarial founding). Founding dues are held in trust by [Datalets.ch](https://datalets.ch/) — per the commercial register a sole proprietorship of Oleg Lavrovsky, not a nonprofit (see the [actor directory](../Evidence/actors-and-landscape.md)); publicai.ch nonetheless describes it as a Swiss nonprofit association.
...
- **Counters and claims.** The homepage counters — 52,293 registered users · 2,166 API developers · 9.58B tokens (2026-07-27; identical on 2026-07-28 — the site is static HTML deployed from GitHub, so these are hardcoded snapshot figures, not live counters) — are **[self-reported]** with scope unstated; *my read:* most plausibly utility-wide rather than Swiss-member counts (10 public RSVPs at founding; no membership number published; a publicai.ch-specific counter cannot be excluded). The homepage headlines **"#1 Apertus deployer worldwide"** beside those counters, and the site repo's [translation-strings file](https://github.com/forpublicai/publicai.ch/blob/main/scripts/i18n_en.json) still contains the answer "largest global deployer of Apertus, with over 45'000 registered users and trillions of tokens processed" — "trillions" conflicts with the 9.58B figure, while 45'000 is an outdated lower bound; the wording was not found on the live pages checked (2026-07-28). The homepage's unlinked **"In collaboration with:"** logo row (ETH AI Center, EPFL AI Center, Swiss {ai} Weeks, …) states no relationship level; counterpart-side, the [Swiss {ai} Weeks 2026 partner roster](https://ai-weeks.ch/2026/partners) lists "Public AI", linking publicai.network — a Public-AI-family tie rather than the cooperative specifically (2026-07-28; an earlier homepage-only check had missed the entry) — while ETH's own Apertus release, the one ETH-side page read directly, contains no publicai.ch statement. No dedicated Swiss press coverage of the cooperative was found (searched swissinfo, NZZ, Netzwoche, Inside IT, heise in DE/EN, 2026-07-27); the nearest is Sanders & Schneier calling SPIU founded ([democracyrenovator.com, 21 Feb 2026](https://www.democracyrenovator.com/p/rewiring-democracy-now-switzerland)) while incorporation was, and remains, pending.
- **Team** (listed 2026-07-27, re-verified 2026-07-28; the standalone team page now redirects into About): Anna Sotnikova, Cornelia Wolfenstädter, Daria Höhener, Joshua Tan, Oleg Lavrovsky, Prashanth Kanduri ([about](https://publicai.ch/about.html)); the earlier co-founder listing of Sabine Wildemann no longer appears (no reason stated publicly; she hosted the founding event and leads aiLights / co-initiated Swiss {ai} Weeks).
...
3. **Switzerland is the institutionalisation test.** Until a notarised founding and a register entry (the draft statutes carry 11 Aug 2026), the cooperative is a membership campaign plus a governance promise — which is also what ordinary Swiss pre-incorporation looks like (draft statutes published, notary engaged, dues in trust). The notarised founding and register entry are the test, not the verdict; a registered Genossenschaft with real members would change its weight in the Swiss landscape.
...
- **Around 11 Aug 2026** (the draft statutes' founding date): does the notarised founding take place; does a Zefix/SHAB entry for "Genossenschaft Public AI" (Liebefeld/Köniz) follow; are founders, board names and a member count published?
